# Design — PHP 8.5 modernization of `freeblog` → `phpenterpriseblog`
...
- **Target project:** `/Users/hidranarias/projects/udemy/phpenterpriseblog` (this repo)
...
│   └── helm/phpenterpriseblog/       Helm chart (Chart, values, templates)
├── docs/
...
| Concern | `freeblog` | `phpenterpriseblog` |
|---|---|---|
...
- ECR repo `phpenterpriseblog`.
- IAM OIDC provider trusting `token.actions.githubusercontent.com`.
- Two OIDC-assumable roles (no long-lived keys):
  - `phpenterpriseblog-ci-ecr` — `ecr:Put*` on `repo/phpenterpriseblog` only, ref-restricted to `main` + tags `v*`.
  - `phpenterpriseblog-cd-eks` — `eks:DescribeCluster` + `secretsmanager:GetSecretValue` on `phpenterpriseblog/*`, tag-restricted to `v*`.
- EKS cluster (≥ 1.30); namespaces `phpenterpriseblog-staging`, `phpenterpriseblog-prod`.
- RDS MySQL 8 per env. Connection in AWS Secrets Manager (`phpenterpriseblog/{env}/db`).
- ElastiCache Redis per env (TLS on). Secret `phpenterpriseblog/{env}/redis`.
...
### 9.4 Helm chart (`deploy/helm/phpenterpriseblog/`)
...
- **Manual:** `helm rollback phpenterpriseblog <REV> -n phpenterpriseblog-prod`.
...
## 10. Migration plan from `freeblog` → `phpenterpriseblog` (high level)
...
- **OQ1 — Project name spelling.** The user wrote `phpenterpriseblog` (not `phpenterpriseblog`). Treated as intentional; will rename if confirmed typo.
- **OQ2 — Domain name.** Spec uses `phpenterpriseblog.example.com` / `staging.phpenterpriseblog.example.com` as placeholders. Replace before first deploy.
